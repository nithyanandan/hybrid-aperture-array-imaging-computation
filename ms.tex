...
We estimate omputational cost based on number of floating point operations and see how this cost is distributed over the 
...
{fig_01.pdf}
\caption{A hybrid multi-stage architecture for aperture arrays consisting of stations each of which contain multiple elements. Black arrows indicate pathways where the data is in the form of electric fields with phase coherence. Gray arrows indicate pathways where the signal is a ``squared'' or a correlated quantity with no absolute but only relative phase information. At the top are intra-station architectures, namely, BF, correlator, and EPIC. The top box denotes intra-station imaging options. The bottom box denotes inter-station imaging architectures which take the coherent outputs from multiple stations with intra-station voltage beamformer and EPIC architectures deployed on each of them. 
