...
Since the computational costs \textbf{of the inter-station processing now} depend on the array layout \textbf{and that of intra-station processing on the station layout}, it necessitates hybrid architectures appropriate for multi-scale aperture arrays. Table~\ref{tab:intra-inter-station-coherent-imaging} summarises these costs. 
...
$\boldsymbol{\sigma}_l$ will denote pixel locations within a selected station's resolution solid angle element centred on $\hat{\boldsymbol{s}}_k$, and $\boldsymbol{r}_{m}$ denotes the centroid location of station $m$. 
