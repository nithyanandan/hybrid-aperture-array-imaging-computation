...
In this approach, $\widetilde{V}_{a_m b_m}^{pq}$, the calibrated and inverse noise covariance weighted visibilities in station, $m$, \textbf{obtained after the correlator} in Equation~(\ref{eqn:intra-station-pol-visibilities}) \textbf{not weighted and beamformed using a DFT. Instead, they} are gridded with weights, $\widetilde{B}_{a_m b_m}^{\alpha\beta;pq*}(\Delta\boldsymbol{r})$, \textbf{onto a grid in the aperture plane}, which is the Fourier transform equivalent of $\widetilde{\mathcal{B}}_{a_m b_m}^{\alpha\beta;pq*}(\hat{\boldsymbol{s}}_k)$ in Equation~(\ref{eqn:intra-station-pol-xbf-img-expl}). \textbf{The gridded visibilities} are Fourier transformed using an FFT rather than the DFT used in correlator beamforming,
\begin{align}
