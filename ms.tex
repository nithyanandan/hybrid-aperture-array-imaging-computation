...
The intra-station coherent combinations $\widetilde{\mathcal{E}}_m^\alpha(\hat{\boldsymbol{s}}_k)$ \textbf{produced through BF and EPIC, respectively} in Equations~(\ref{eqn:intra-station-pol-hol-img-expl}) and (\ref{eqn:intra-station-pol-hol-img-epic}), effectively \textbf{convert the station, $m$, to act as multiple virtual station-sized} telescopes measuring the electric fields but simultaneously pointed towards multiple locations, denoted by $\hat{\boldsymbol{s}}_k$.
...
Using these electric fields as \textbf{the input} measurements from virtual telescopes \textbf{pointed towards any given direction, $\hat{\boldsymbol{s}}_k$, synthesised from the stations}, the inter-station imaging architecture can employ any of the architectures that were \textbf{previously discussed for intra-station processing. The same can be performed for all possible directions filling the entire field of view.}
...
Since the computational costs \textbf{of the inter-station processing now} depend on the array layout \textbf{and that of intra-station processing on the station layout}, it necessitates hybrid architectures appropriate for multi-scale aperture arrays. 
...
$\boldsymbol{\sigma}_l$ will denote pixel locations within a selected station's resolution solid angle element centred on $\hat{\boldsymbol{s}}_k$, and $\boldsymbol{r}_{m}$ denotes the centroid location of station $m$. \textbf{Table~\ref{tab:intra-inter-station-coherent-imaging} summarises the computational costs of inter-station processing.} 
...
is given by the \textbf{outer product over polarisation states} (``squaring'') and averaging, similar to Equation~(\ref{eqn:intra-station-opt-pol-img-outprod}),
\begin{align}
...
\textbf{Instead of DFT beamforming with synthesised electric fields from intra-station processing,} inter-station EPIC \textbf{can be used to simultaneously form beams in all directions filling the available field of view using FFT.} It takes the form
\begin{align}
...
\textbf{In this correlator-based approach,} the synthesised intra-station electric fields,
% $\widetilde{\boldsymbol{\mathcal{E}}}_{[m,p,\hat{\boldsymbol{s}}]}$ or
$\widetilde{\mathcal{E}}_m^p(\hat{\boldsymbol{s}}_k)$, phase-centred towards $\hat{\boldsymbol{s}}_k$, are cross-correlated \textbf{and temporally averaged} to produce the inter-station visibilities, 
\begin{align}
