...
\begin{tablenotes}[hang]
% \item[]Table note
\item[a]$N_\textrm{ke}$ and $N_\textrm{ks}$ denote the sizes (in number of cells) the gridding kernels of the antenna elements and the stations, respectively. In this paper, $N_\textrm{ke}=1^2$ and $N_\textrm{ks}=1^2$ to remove the aliasing effects within the respective fields of view. For XFFT, gridding sizes for intra- and inter-station correlations are quadrupled to $2^2 N_\textrm{ke}$ and $2^2 N_\textrm{ks}$, respectively compared to pre-correlated data used in EPIC. \textbf{If the gridding kernels incorporate $w$-projection, the kernel sizes will be correspondingly larger.}
